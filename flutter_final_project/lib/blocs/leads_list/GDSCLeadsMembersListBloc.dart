...
class GDSCLeadsMembersListBloc
    extends Bloc<GDSCLeadsMembersListEvent, GDSCLeadsMembersListState> {
  ...
  GDSCLeadsMembersListBloc({required this.gDSCLeadsMembersRepository})
      : super(GDSCLeadsMembersListInitialState()) {
    @override
    ...
    on<CreateGDSCLeadsMembersListEvent>((event, emit) async {
      try {
        if (state is GDSCLeadsMembersListSuccessOrLoadedState) {
          await gDSCLeadsMembersRepository
              .createAppUserFunctionInGDSCLeadsMembersListRepositoryFile(
                  name: event.name, email: event.email, role: event.role);
        }
      } catch (e) {
        emit(const GDSCLeadsMembersListErrorState(
            'GDSCLeadsMembersList creation failed.'));
      }
    });
    ...
    on<ReadOneGDSCLeadsMembersListEvent>((event, emit) {
      try {
        emit(GDSCLeadsMembersListLoadingState());
        final user = gDSCLeadsMembersRepository
            .readOneAppUserFunctionInGDSCLeadsMembersListRepositoryFile(
                event.userId);
        emit(OneGDSCLeadsMembersListSuccessOrLoadedState(user));
      } catch (e) {
        emit(const GDSCLeadsMembersListErrorState(
            'GDSCLeadsMembersList loading/reading failed.'));
      }
    });
    ...
    on<ReadAllGDSCLeadsMembersListEvent>((event, emit) {
      try {
        emit(GDSCLeadsMembersListLoadingState());
        Stream<List<AppUser>> streamResponse = gDSCLeadsMembersRepository
            .allGDSCLeadsMembersListFunctionInGDSCLeadsMembersListRepositoryFile();
    ...
    on<ReadAllAdminsListEvent>((event, emit) {
      try {
        emit(GDSCLeadsMembersListLoadingState());
        Stream<List<AppUser>> streamResponse = gDSCLeadsMembersRepository
            .allAdminsListFunctionInGDSCLeadsMembersListRepositoryFile();
        emit(GDSCLeadsMembersListSuccessOrLoadedState(streamResponse));
      } catch (e) {
        emit(const GDSCLeadsMembersListErrorState(
            'GDSCLeadsMembersList loading/reading failed.'));
    ...
    on<ReadAllApprovedGDSCLeadsListEvent>((event, emit) {
      try {
        emit(GDSCLeadsMembersListLoadingState());
        Stream<List<AppUser>> streamResponse = gDSCLeadsMembersRepository
            .allApprovedGDSCLeadsListFunctionInGDSCLeadsMembersListRepositoryFile();
        emit(GDSCLeadsMembersListSuccessOrLoadedState(streamResponse));
      } catch (e) {
        emit(const GDSCLeadsMembersListErrorState(
            'ApprovedGDSCLeadList loading/reading failed.'));
      }
    });
    ...
    on<ReadAllPendingGDSCLeadsListEvent>((event, emit) {
      try {
        emit(GDSCLeadsMembersListLoadingState());
        Stream<List<AppUser>> streamResponse = gDSCLeadsMembersRepository
            .allPendingGDSCLeadsListFunctionInGDSCLeadsMembersListRepositoryFile();
        emit(GDSCLeadsMembersListSuccessOrLoadedState(streamResponse));
      } catch (e) {
        emit(const GDSCLeadsMembersListErrorState(
            'PendingGDSCLeadList loading/reading failed.'));
      }
    });
    ...
    on<ApproveGDSCLeadEvent>((event, emit) async {
      try {
        if (state is GDSCLeadsMembersListSuccessOrLoadedState) {
          await gDSCLeadsMembersRepository
              .approveGDSCLeadFunctionInGDSCLeadsMembersListRepositoryFile(
                  event.user);
        }
      } catch (e) {
        emit(const GDSCLeadsMembersListErrorState(
            'GDSCLeadsMembersList update-ion failed.'));
      }
    });
    ...
    on<RemoveGDSCLeadEvent>((event, emit) async {
      try {
        if (state is GDSCLeadsMembersListSuccessOrLoadedState) {
          await gDSCLeadsMembersRepository
              .removeGDSCLeadFunctionInGDSCLeadsMembersListRepositoryFile(
                  event.user);
        }
      } catch (e) {
        emit(const GDSCLeadsMembersListErrorState(
    ...
    on<UpdateGDSCLeadsMembersListEvent>((event, emit) async {
      try {
        if (state is GDSCLeadsMembersListSuccessOrLoadedState) {
          await gDSCLeadsMembersRepository
              .updateAppUserFunctionInGDSCLeadsMembersListRepositoryFile(
                  event.user);
        }
      } catch (e) {
        emit(const GDSCLeadsMembersListErrorState(
    ...
    on<DeleteGDSCLeadsMembersListEvent>((event, emit) async {
      try {
        if (state is GDSCLeadsMembersListSuccessOrLoadedState) {
          await gDSCLeadsMembersRepository
              .deleteAppUserFunctionInGDSCLeadsMembersListRepositoryFile(
                  event.userId);
        }
      } catch (e) {
        emit(const GDSCLeadsMembersListErrorState(
            'GDSCLeadsMembersList deletion failed.'));
      }
    });
  }
}
